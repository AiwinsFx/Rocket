    ...
    public class GenericEventNameAttribute : Attribute, IEventNameProvider {
        ...
        public virtual string GetName (Type eventType) {
            if (!eventType.IsGenericType) {
                throw new RocketException ($"Given type is not generic: {eventType.AssemblyQualifiedName}");
            }

            var genericArguments = eventType.GetGenericArguments ();
            if (genericArguments.Length > 1) {
                throw new RocketException ($"Given type has more than one generic argument: {eventType.AssemblyQualifiedName}");
            }
