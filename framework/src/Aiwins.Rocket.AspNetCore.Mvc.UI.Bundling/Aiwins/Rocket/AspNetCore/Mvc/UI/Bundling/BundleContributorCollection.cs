...
namespace Aiwins.Rocket.AspNetCore.Mvc.UI.Bundling
{
    public class BundleContributorCollection
    {
        ...
        private void AddInstanceToContributors(Type contributorType)
        {
            ...
            {
                throw new RocketException($"Given {nameof(contributorType)} ({contributorType.AssemblyQualifiedName}) should implement the {typeof(IBundleContributor).AssemblyQualifiedName} interface!");
            }
            ...
            {
                throw new RocketException($"Can not instantiate {contributorType.AssemblyQualifiedName}", ex);
            }
        }
    }
}