...
namespace Aiwins.Rocket.AspNetCore.Mvc.UI.Bundling
{
    public class BundleConfigurationCollection
    {
        ...
        /// Throws <see cref="RocketException"/> if there is already a bundle with the same name.
        /// </summary>
            ...
            [CanBeNull] Action<BundleConfiguration> configureAction = null)
        {
            ...
            {
                throw new RocketException($"There is already a bundle added with given {nameof(bundleName)}: {bundleName}");
            }
        ...
        public BundleConfiguration Get([NotNull] string bundleName)
        {
            ...
            if (!_bundles.TryGetValue(bundleName, out var bundle))
            {
                throw new RocketException($"There is no bundle with given {nameof(bundleName)}: {bundleName}");
            }
