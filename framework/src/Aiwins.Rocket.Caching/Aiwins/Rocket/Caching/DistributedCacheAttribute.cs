...
namespace Penguin.Core.Caching {
    ...
    public class DistributedCacheAttribute : AbstractInterceptorAttribute {
        private static readonly MethodInfo TaskResultMethod;
        private readonly AsyncLock _lock = new AsyncLock ();

        static DistributedCacheAttribute () {
        ...
        /// 缓存有限期，单位：分钟。默认值：60。
        /// </summary>
        public long Expiration { get; set; } = 60;
        /// <summary>
        ...
        [FromServiceContext]
        public ILogger<DistributedCacheAttribute> Logger { get; set; }
        ...
        public override async Task Invoke (AspectContext context, AspectDelegate next) {
            ...
            if (parameters.Any (it => it.IsIn || it.IsOut)) {
                ...
                try {
                    var cacheValue = await CacheClient.GetAsync<string> (key);
                    if (cacheValue.HasValue) {
                        var resultValue = JsonConvert.DeserializeObject (cacheValue.Value, returnType);
                        ...
                        if (!ThreadLock) {
                            ...
                            using (await _lock.LockAsync ()) {
                                cacheValue = await CacheClient.GetAsync<string> (key);
                                if (cacheValue.HasValue) {
                                    var resultValue = JsonConvert.DeserializeObject (cacheValue.Value, returnType);
                                    context.ReturnValue = context.IsAsync () ?
                                        TaskResultMethod.MakeGenericMethod (returnType).Invoke (null, new object[] { resultValue }) :
        ...
        public async Task GetActionResultAndSetCacheValue (AspectContext context, AspectDelegate next, string key, Type type) {
            ...
            if (result != null)
                CacheClient.SetAsync (key, result, TimeSpan.FromMinutes (Expiration));
        }
    }
}