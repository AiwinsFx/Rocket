...
{
    public class RocketTabsTagHelperService : RocketTagHelperService<RocketTabsTagHelper>
    {
        ...
        protected virtual void SetActiveTab(List<TabItem> items)
        {
            ...
            foreach (var tabItem in items)
            {
                if (tabItem.Active)
                {
                    tabItem.Content = tabItem.Content.Replace(RocketTabItemShowActivePlaceholder, " show active");
                    tabItem.Header = tabItem.Header.Replace(RocketTabItemActivePlaceholder, " active").Replace(RocketTabItemSelectedPlaceholder, "true");
                }
                else
                {
                    tabItem.Content = tabItem.Content.Replace(RocketTabItemShowActivePlaceholder, "");
                    tabItem.Header = tabItem.Header.Replace(RocketTabItemActivePlaceholder, "").Replace(RocketTabItemSelectedPlaceholder, "false");
                }
            }
        ...
        protected virtual string GetHeaders(TagHelperContext context, TagHelperOutput output, List<TabItem> items)
        {
            ...
            for (var index = 0; index < items.Count; index++)
            {
                ...
                if (item.IsDropdown)
                {
                    ...
                    header = item.Header.Replace(RocketTabDropdownItemsActivePlaceholder, childHeadersAsString);
                }
