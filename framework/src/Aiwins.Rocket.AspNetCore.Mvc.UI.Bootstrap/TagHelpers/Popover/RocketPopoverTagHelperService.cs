...
{
    public class RocketPopoverTagHelperService : RocketTagHelperService<RocketPopoverTagHelper>
    {
        ...
        protected virtual string GetDataContent()
        {
            switch (GetDirectory())
            {
                case PopoverDirectory.Top:
                    return TagHelper.RocketPopoverTop;
                case PopoverDirectory.Right:
                    return TagHelper.RocketPopoverRight;
                case PopoverDirectory.Bottom:
                    return TagHelper.RocketPopoverBottom;
                case PopoverDirectory.Left:
                    return TagHelper.RocketPopoverLeft;
                default:
                    return TagHelper.RocketPopover;
            }
        }

        protected virtual PopoverDirectory GetDirectory()
        {
            if (!string.IsNullOrWhiteSpace(TagHelper.RocketPopoverTop))
            {
                return PopoverDirectory.Top;
            }
            if (!string.IsNullOrWhiteSpace(TagHelper.RocketPopoverBottom))
            {
                return PopoverDirectory.Bottom;
            }
            if (!string.IsNullOrWhiteSpace(TagHelper.RocketPopoverRight))
            {
                return PopoverDirectory.Right;
            }
            if (!string.IsNullOrWhiteSpace(TagHelper.RocketPopoverLeft))
            {
