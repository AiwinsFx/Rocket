...
namespace Aiwins.Rocket.IdentityModel {
    ...
    public class IdentityModelAuthenticationService : IIdentityModelAuthenticationService, ITransientDependency {
        ...
        protected RocketIdentityClientOptions ClientOptions { get; }
        protected ICancellationTokenProvider CancellationTokenProvider { get; }
        protected IHttpClientFactory HttpClientFactory { get; }

        public IdentityModelAuthenticationService (
            IOptions<RocketIdentityClientOptions> options,
        ...
        public virtual async Task<string> GetAccessTokenAsync (IdentityClientConfiguration configuration) {
            ...
            if (discoveryResponse.IsError) {
                throw new RocketException ($"Could not retrieve the OpenId Connect discovery document! ErrorType: {discoveryResponse.ErrorType}. Error: {discoveryResponse.Error}");
            }
            ...
            if (tokenResponse.IsError) {
                if (tokenResponse.ErrorDescription != null) {
                    throw new RocketException ($"Could not get token from the OpenId Connect server! ErrorType: {tokenResponse.ErrorType}. Error: {tokenResponse.Error}. ErrorDescription: {tokenResponse.ErrorDescription}. HttpStatusCode: {tokenResponse.HttpStatusCode}");
                }
                ...
                throw new RocketException (withoutInnerException[0]);
            }
