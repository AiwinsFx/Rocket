...
namespace Aiwins.Rocket.BackgroundJobs {
    public class BackgroundJobExecuter : IBackgroundJobExecuter, ITransientDependency {
        ...
        public BackgroundJobExecuter (IOptions<RocketBackgroundJobOptions> options) {
        ...
        public virtual async Task ExecuteAsync (JobExecutionContext context) {
            ...
            if (job == null) {
                throw new RocketException ("The job type is not registered to DI: " + context.JobType);
            }
            ...
            if (jobExecuteMethod == null) {
                throw new RocketException ($"Given job type does not implement {typeof(IBackgroundJob<>).Name} or {typeof(IAsyncBackgroundJob<>).Name}. " +
