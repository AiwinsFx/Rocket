    ...
    public class SolutionModuleAdder : ITransientDependency {
        ...
        private async Task DeleteAppAndDemoFolderAsync (string targetModuleFolder) {
            ...
            if (Directory.Exists (appFolder)) {
                await Task.Run (() => Directory.Delete (appFolder, true));
            }

            var demoFolder = Path.Combine (targetModuleFolder, "demo");
            if (Directory.Exists (demoFolder)) {
                await Task.Run (() => Directory.Delete (demoFolder, true));
            }
        }
        ...
        protected void ModifyDbContext (string[] projectFiles, ModuleInfo module, string startupProject, bool skipDbMigrations = false) {
            ...
            var dbContextFile = DerivedClassFinder.Find (dbMigrationsProject, "RocketDbContext").FirstOrDefault ();

            if (dbContextFile == null) {
                Logger.LogDebug ($"{dbMigrationsProject} project doesn't have a class that is derived from \"RocketDbContext\".");
                return;
            }
        ...
        protected virtual async Task<ModuleWithMastersInfo> FindModuleInfoAsync (string moduleName) {
            using (var client = new CliHttpClient ()) {
                var url = $"{CliUrls.WwwRocketIo}api/app/module/byNameWithDetails/?name=" + moduleName;
        ...
        protected virtual async Task<bool> IsProjectTiered (string[] projectFiles) {
            return await Task.Run (() => projectFiles.Select (ProjectFileNameHelper.GetAssemblyNameFromProjectPath).Any (p => p.EndsWith (".IdentityServer") || p.EndsWith (".HttpApi.Host")));
        }
    }
}