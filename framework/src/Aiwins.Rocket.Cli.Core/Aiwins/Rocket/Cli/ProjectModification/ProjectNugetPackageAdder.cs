    ...
    public class ProjectNugetPackageAdder : ITransientDependency
    {
        ...
        public Task AddAsync(string projectFile, NugetPackageInfo package)
        {
            ...
            using (DirectoryHelper.ChangeCurrentDirectory(Path.GetDirectoryName(projectFile)))
            {
                ...
                var moduleFiles = ModuleClassFinder.Find(projectFile, "RocketModule");
                if (moduleFiles.Count == 0)
                {
                    throw new CliUsageException($"Could not find a class derived from RocketModule in the project {projectFile}");
                }

                if (moduleFiles.Count > 1)
                {
                    throw new CliUsageException($"There are multiple classes derived from RocketModule in the project {projectFile}: " + moduleFiles.JoinAsString(", "));
                }
        ...
        protected virtual async Task<NugetPackageInfo> FindNugetPackageInfoAsync(string moduleName)
        {
            ...
            {
                var url = $"{CliUrls.WwwRocketIo}api/app/nugetPackage/byName/?name=" + moduleName;
