...
{
    public class AiwinsNugetPackagesVersionUpdater : ITransientDependency
    {
        ...
        public AiwinsNugetPackagesVersionUpdater(NuGetService nuGetService, MyGetPackageListFinder myGetPackageListFinder)
        {
        ...
        protected virtual async Task UpdateInternalAsync(string projectPath, bool includePreviews = false, bool switchToStable = false)
        {
            ...
            var updatedContent = await UpdateAiwinsPackagesAsync(fileContent, includePreviews, switchToStable);

            File.WriteAllText(projectPath, updatedContent);
        }

        private async Task<string> UpdateAiwinsPackagesAsync(string content, bool includePreviews = false, bool switchToStable = false)
        {
            ...
            try
            {
                ...
                var packageNodeList = doc.SelectNodes("/Project/ItemGroup/PackageReference[starts-with(@Include, 'Aiwins.')]");
            ...
            {
                Logger.LogError("Cannot update Aiwins.* packages! An error occured while updating the package \"{0}\". Error: {1}", packageId, ex.Message);
