...
namespace Aiwins.Rocket.Cli
{
    public class CliService : ITransientDependency
    {
        ...
        private static async Task<SemanticVersion> GetCurrentCliVersion(Assembly assembly)
        {
            ...
            {
                if (line.StartsWith("aiwins.rocket.cli", StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        private async Task<SemanticVersion> GetLatestVersion(UpdateChannel updateChannel)
        {
            switch (updateChannel)
            {
                case UpdateChannel.Stable:
                    return await NuGetService.GetLatestVersionOrNullAsync("Aiwins.Rocket.Cli");

                case UpdateChannel.Prerelease:
                    return await NuGetService.GetLatestVersionOrNullAsync("Aiwins.Rocket.Cli", includePreviews: true);

                case UpdateChannel.Nightly:
                    return await NuGetService.GetLatestVersionOrNullAsync("Aiwins.Rocket.Cli", includeNightly: true);
        ...
        private void LogNewVersionInfo(UpdateChannel updateChannel, SemanticVersion latestVersion, string toolPath)
        {
            ...
            {
                case UpdateChannel.Stable:
                    Logger.LogWarning($"dotnet tool update {toolPathArg} Aiwins.Rocket.Cli");
                    break;

                case UpdateChannel.Prerelease:
                    Logger.LogWarning($"dotnet tool uninstall {toolPathArg} Aiwins.Rocket.Cli");
                    Logger.LogWarning($"dotnet tool install {toolPathArg} Aiwins.Rocket.Cli --version {latestVersion}");
                    break;

                case UpdateChannel.Nightly:
                case UpdateChannel.Development:
                    Logger.LogWarning($"dotnet tool uninstall {toolPathArg} Aiwins.Rocket.Cli");
                    Logger.LogWarning($"dotnet tool install {toolPathArg} Aiwins.Rocket.Cli --add-source https://www.myget.org/F/rocket-nightly/api/v3/index.json --version {latestVersion}");
                    break;
                default:
