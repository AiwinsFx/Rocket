...
@Component({
  selector: 'rocket-validation-error',
  template: `
    <div class="invalid-feedback" *ngFor="let error of rocketErrors; trackBy: trackByFn">
      {{ error.message | rocketLocalization: error.interpoliteParams }}
    </div>
  `,
...
export class ValidationErrorComponent extends ErrorComponent {
  get rocketErrors(): Validation.Error[] & { interpoliteParams?: string[] } {
