...
describe('ModalComponent', () => {
  ...
  beforeEach(async () => {
    ...
    spectator = createHost(
      `<rocket-modal [(visible)]="visible" [busy]="busy" [centered]="true" (appear)="appearFn()" (disappear)="disappearFn()" size="sm" modalClass="test">
        ...
        <ng-template #rocketFooter>
          <div class="footer">
            <button id="rocket-close" #rocketClose></button>
            <rocket-button>Submit</rocket-button>
          </div>
        </ng-template>
      </rocket-modal>
      `,
  ...
  it('should project its template to rocket-modal-container', () => {
    const modal = selectModal();
    expect(modal).toBeTruthy();
    expect(modal.querySelector('div.modal-backdrop')).toBeTruthy();
    expect(modal.querySelector('div#rocket-modal-dialog')).toBeTruthy();
  });
  ...
  xit('should close with the rocketClose', async () => {
    await wait0ms();

    spectator.dispatchMouseEvent(spectator.component.rocketClose, 'click');
...
function selectModal(modalSelector = ''): Element {
  return document.querySelector(`rocket-modal-container div.modal${modalSelector}`);
}
