...
describe('ButtonComponent', () => {
  ...
  beforeEach(
    () =>
      (spectator = createHost('<rocket-button iconClass="fa fa-check" [attributes]="attributes">Button</rocket-button>', {
        hostProps: { attributes: { autofocus: '', name: 'rocket-button' } },
      })),
  );
  ...
  it('should disabled when the loading input is true', () => {
    expect(spectator.query('[autofocus][name="rocket-button"]')).toBeTruthy();
  });
});
