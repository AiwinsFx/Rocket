...
describe('ErrorHandler', () => {
  ...
  test('should display HttpErrorWrapperComponent when server error occurs', () => {
    ...
    const params = {
      title: {
        key: 'RocketAccount::500Message',
        defaultValue: DEFAULT_ERROR_MESSAGES.defaultError500.title,
      },
      details: {
        key: 'RocketAccount::InternalServerErrorMessage',
  ...
  test('should display HttpErrorWrapperComponent when authorize error occurs', () => {
    ...
    const params = {
      title: {
        key: 'RocketAccount::DefaultErrorMessage403',
        defaultValue: DEFAULT_ERROR_MESSAGES.defaultError403.title,
      },
      details: {
        key: 'RocketAccount::DefaultErrorMessage403Detail',
  ...
  test('should display HttpErrorWrapperComponent when unknown error occurs', () => {
    ...
    const params = {
      title: {
        key: 'RocketAccount::DefaultErrorMessage',
  ...
  test('should call error method of ConfirmationService when not found error occurs', () => {
    ...
    expect(errorConfirmation).toHaveBeenCalledWith(
      {
        key: 'RocketAccount::DefaultErrorMessage404',
        defaultValue: DEFAULT_ERROR_MESSAGES.defaultError404.details,
      },
      {
        key: 'RocketAccount::DefaultErrorMessage404Detail',
  ...
  test('should call error method of ConfirmationService when authenticated error occurs', () => {
    ...
    expect(errorConfirmation).toHaveBeenCalledWith(
      {
        key: 'RocketAccount::DefaultErrorMessage401',
        defaultValue: DEFAULT_ERROR_MESSAGES.defaultError401.title,
      },
      {
        key: 'RocketAccount::DefaultErrorMessage401Detail',
  ...
  test('should call error method of ConfirmationService when authenticated error occurs with _RocketErrorFormat header', done => {
    spectator
      .get(Actions)
    ...
    const headers: HttpHeaders = new HttpHeaders({
      _RocketErrorFormat: '_RocketErrorFormat',
    });
  ...
  test('should call error method of ConfirmationService when error occurs with _RocketErrorFormat header', () => {
    let headers: HttpHeaders = new HttpHeaders();
    headers = headers.append('_RocketErrorFormat', '_RocketErrorFormat');
    store.dispatch(
      new RestOccurError(
...
function removeIfExistsInDom(errorSelector: () => HTMLDivElement | null) {
  const rocketError = errorSelector();
  if (rocketError) rocketError.parentNode.removeChild(rocketError);
}

function selectHtmlErrorWrapper(): HTMLDivElement | null {
  return document.querySelector('rocket-http-error-wrapper');
}

function selectCustomError(): HTMLDivElement | null {
  return document.querySelector('rocket-dummy-error');
}
