...
describe('ForDirective', () => {
  ...
  describe('basic', () => {
    beforeEach(() => {
      spectator = createDirective('<ul><li  *rocketFor="let item of items">{{ item }}</li></ul>', {
        hostProps: { items },
      });
  ...
  describe('trackBy', () => {
    ...
    beforeEach(() => {
      spectator = createDirective('<ul><li  *rocketFor="let item of items; trackBy: trackByFn">{{ item }}</li></ul>', {
  ...
  describe('with order', () => {
    beforeEach(() => {
      spectator = createDirective(
        `<ul>
          <li
            *rocketFor="let item of [{value: 3}, {value: 6}, {value: 2}];
            orderBy: 'value';
            orderDir: orderDir">
            {{ item.value }}
          </li>
  ...
  describe('with filter', () => {
    beforeEach(() => {
      spectator = createDirective(
        `<ul>
          <li
            *rocketFor="let item of [{value: 'test'}, {value: 'rocket'}, {value: 'aiwins'}];
            filterBy: 'value';
            filterVal: filterVal">
            {{ item.value }}
          </li>
    ...
    test('should not filter when filterVal is empty,', () => {
      const elements = spectator.queryAll('li');
      expect(elements.map(el => el.textContent.trim())).toEqual(['test', 'rocket', 'aiwins']);
    });

    test('should be filtered', () => {
      (spectator.hostComponent as any).filterVal = 'aiwins';
      spectator.detectChanges();

      expect(spectator.query('li')).toHaveText('aiwins');
    });

    test('should not show an element when filter value not match to any text', () => {
      (spectator.hostComponent as any).filterVal = 'aiwinss';
      spectator.detectChanges();
