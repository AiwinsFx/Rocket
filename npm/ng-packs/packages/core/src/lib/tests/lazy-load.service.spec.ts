...
describe('LazyLoadService (Deprecated)', () => {
  ...
  test('should load script with content just one time', done => {
    ...
    service.load('https://aiwins.cn', 'script', 'test').subscribe(res => {
      expect(
        document.querySelector('script[src="https://aiwins.cn"][type="text/javascript"]').textContent,
      ).toMatch('test');
    });
    ...
    service.load('https://aiwins.cn', 'script', 'test').subscribe(res => {
      expect(
        document.querySelectorAll('script[src="https://aiwins.cn"][type="text/javascript"]'),
      ).toHaveLength(1);
      done();
    });
  });
  ...
  describe('style with url', () => {
    ...
    test('should load an link element', done => {
      service.load('https://aiwins.cn', 'style').subscribe(res => {
        expect(
          document.querySelector('link[type="text/css"][rel="stylesheet"][href="https://aiwins.cn"]'),
        ).toBeTruthy();
        done();
      });
    ...
    test('should load link elements', done => {
      service.load(['https://aiwins.cn', 'https://aiwinssoft.com'], 'style').subscribe(res => {
