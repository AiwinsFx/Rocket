...
@Component({
  selector: 'rocket-layout-application',
...
describe('DynamicLayoutComponent', () => {
  ...
  it('should handle application layout from parent rocket route and display it', async () => {
    spectator.router.navigateByUrl('/parentWithLayout/childWithoutLayout');
    await spectator.fixture.whenStable();
    spectator.detectComponentChanges();
    expect(spectator.query('rocket-dynamic-layout')).toBeTruthy();
    expect(spectator.query('rocket-layout-application')).toBeTruthy();
  });
  ...
  it('should handle account layout from own property and display it', async () => {
    ...
    await spectator.fixture.whenStable();
    spectator.detectComponentChanges();
    expect(spectator.query('rocket-layout-account')).toBeTruthy();
  });
  ...
  it('should handle empty layout from route data and display it', async () => {
    ...
    await spectator.fixture.whenStable();
    spectator.detectComponentChanges();
    expect(spectator.query('rocket-layout-empty')).toBeTruthy();
  });
  ...
  it('should not display any layout when layouts are empty', async () => {
    ...
    expect(spectator.query('rocket-layout-empty')).toBeFalsy();
    expect(spectator.query('rocket-dynamic-layout').children[0].tagName).toEqual('ROUTER-OUTLET');
  });
});
